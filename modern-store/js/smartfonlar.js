document.addEventListener("DOMContentLoaded", () => {
  ...
  const smartphonesGrid2 = document.getElementById("smartphones-grid-2")
  const notificationClose = document.getElementById("notification-close")
  const warningClose = document.getElementById("warning-close")
  const recommendedGrid = document.getElementById("recommended-grid")
  const smartphoneLinks = document.querySelectorAll(".smartphone-link")
  ...
  function initEventListeners() {
    ...
    // Smartphone links
    smartphoneLinks.forEach(link => {
      link.addEventListener("click", (e) => {
        e.preventDefault()
        document.querySelector(".smartphones-section").scrollIntoView({
          behavior: "smooth"
        })
      })
    })
  ...
  function openProductDetailModal(productId) {
    ...
    const phone = smartphones.find(p => p.id === productId)
    if (!phone) return
    ...
    breadcrumbContainer.innerHTML = `
      <div class="breadcrumb">
        ...
        <a href="#" onclick="closeProductDetailModal()">Smartfonlar</a>
        <span class="breadcrumb-separator">&gt;</span>
        <span class="breadcrumb-current">${phone.name}</span>
      </div>
    `
    ...
    productDetailContent.innerHTML = `
      <div class="product-detail">
        <div class="product-detail-gallery">
          <div class="product-detail-badges">
            ${phone.isNew ? '<span class="product-badge badge-new">Yangi</span>' : ''}
            ${phone.originalPrice ? '<span class="product-badge badge-sale">Chegirma</span>' : ''}
          </div>
        ...
        <div class="product-detail-info">
          <h1 class="product-detail-title">${phone.name}</h1>
          <div class="product-detail-rating">
            <div class="rating-stars">
              ${generateStars(phone.rating, true)}
            </div>
            <span class="rating-count">(${phone.reviewCount})</span>
            <span class="product-detail-reviews">Sharhlar (${phone.reviewCount})</span>
          </div>
          ...
          <div class="product-detail-description">
            ...
            <p>Bu smartfon haqida batafsil ma'lumot. Mahsulotning asosiy xususiyatlari, afzalliklari va qo'shimcha ma'lumotlar.</p>
            <ul class="product-detail-features">
              <li><i class="fas fa-check"></i> Yuqori sifatli kamera</li>
          ...
          <div class="product-detail-actions">
            <button class="detail-add-to-cart" data-id="${phone.id}">Savatga qo'shish</button>
            <button class="detail-buy-now" data-id="${phone.id}">Hozir sotib olish</button>
            <button class="detail-wishlist" data-id="${phone.id}"><i class="far fa-heart"></i></button>
          </div>
      ...
      <div class="product-detail-tabs">
        <div class="tabs-header">
          ...
          <button class="tab-btn" data-tab="reviews">Sharhlar (${phone.reviewCount})</button>
        </div>
        <div class="tabs-content">
          ...
          <div class="tab-panel" id="specifications">
            ...
            <ul>
              <li><strong>Brend:</strong> ${phone.name.split(' ')[0]}</li>
              <li><strong>Model:</strong> ${phone.name}</li>
              <li><strong>Xotira:</strong> 128GB/256GB/512GB</li>
              <li><strong>RAM:</strong> 8GB/12GB</li>
              <li><strong>Kamera:</strong> 48MP asosiy kamera</li>
              <li><strong>Batareya:</strong> 4500mAh</li>
              <li><strong>Operatsion tizim:</strong> Android/iOS</li>
            </ul>
          </div>
          <div class="tab-panel" id="reviews">
            ...
            <div class="review">
              ...
              <p>Zo'r telefon! Kamera sifati ajoyib, batareya uzoq vaqt ishlaydi. Tavsiya qilaman!</p>
            </div>
            <div class="review">
              <div class="review-header">
                <h4>Malika Tosheva</h4>
                <div class="rating-stars">
                  ${generateStars(4)}
                </div>
              </div>
              <p>Yaxshi telefon, lekin narxi biroz qimmat. Sifati esa zo'r.</p>
            </div>
          </div>
        </div>
      </div>
    ...
    addProductDetailEventListeners(phone)

    // Render recommended products
    renderRecommendedProducts(phone.id)
  ...
  function addProductDetailEventListeners(phone) {
  ...
  function renderRecommendedProducts(currentPhoneId) {
    if (!recommendedGrid) return

    // Get 4 random smartphones excluding the current one
    const otherPhones = smartphones.filter(phone => phone.id !== currentPhoneId)
    const shuffled = otherPhones.sort(() => 0.5 - Math.random())
    const recommended = shuffled.slice(0, 4)

    recommendedGrid.innerHTML = ""
    recommended.forEach(phone => {
      const phoneCard = createPhoneCard(phone)
      phoneCard.classList.add("recommended-card")
      recommendedGrid.appendChild(phoneCard)
    })
  }
  ...
  function renderSmartphones() {
    if (!smartphonesGrid || !smartphonesGrid2) return

    // Filter smartphones
    let filteredSmartphones = smartphones
    
    if (currentFilter === "new") {
      filteredSmartphones = smartphones.filter(phone => phone.isNew)
    } else if (currentFilter === "discount") {
      filteredSmartphones = smartphones.filter(phone => phone.originalPrice)
    } else if (currentFilter === "popular") {
      filteredSmartphones = smartphones.filter(phone => phone.isPopular)
    }

    // Split smartphones into two grids
    const firstHalf = filteredSmartphones.slice(0, Math.ceil(filteredSmartphones.length / 2))
    const secondHalf = filteredSmartphones.slice(Math.ceil(filteredSmartphones.length / 2))

    // Render first grid
    smartphonesGrid.innerHTML = ""
    firstHalf.forEach(phone => {
      const phoneCard = createPhoneCard(phone)
      smartphonesGrid.appendChild(phoneCard)
    })

    // Render second grid
    smartphonesGrid2.innerHTML = ""
    secondHalf.forEach(phone => {
      const phoneCard = createPhoneCard(phone)
      smartphonesGrid2.appendChild(phoneCard)
    })
  }

  function createPhoneCard(phone) {
    const phoneCard = document.createElement("div")
    phoneCard.className = "product-card"
    phoneCard.dataset.id = phone.id

    const badgeHTML = phone.isNew ? '<span class="product-badge badge-new">Yangi</span>' : 
                     phone.originalPrice ? '<span class="product-badge badge-sale">Chegirma</span>' : ''
